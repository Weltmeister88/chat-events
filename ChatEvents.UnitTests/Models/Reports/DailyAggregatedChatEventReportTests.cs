...
[TestFixture]
public class DailyAggregatedChatEventReportTests
{
    [Test]
    public void GetTime_ShouldReturnFormattedDate()
    {
        // Arrange
        ...
        var report = new DailyAggregatedChatEventReport(groupedEvents);

        // Act
        string time = report.GetTime();

        // Assert
        Assert.That(dateTimeOffset.ToLocalTime().ToString("yyyy-MM-dd"), Is.EqualTo(time));
    }
